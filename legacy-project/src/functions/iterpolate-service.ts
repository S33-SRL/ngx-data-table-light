...
export class InterpolateService {
    ...
    constructor (currencyPipe:CurrencyPipe, lang:string = 'it')
    {
        ...
        this.functions["FromOther"] = this.intFromOtherData;
    ...
    public arrayFirstByField(array:any[],key:string,fieldName:string[]){
    ...
    public parserStringNasted(template:string, data:any,otherData:any=null, selectorOpen = '{', selectorClose = '}'): any {
        let result =  template+'';
            ...
            try {
                ...
                if (change.indexOf('!@') === 0 && change.indexOf('|') > 0) {
                    ...
                    if(change.indexOf(selectorOpen)>=0){
                        change = this.parserStringNasted(x,data, otherData, selectorOpen, selectorClose);
                    }
                    if (change.indexOf('@') === 0 && change.indexOf('|') > 0) {
                        let parameters = change.split('|');
                ...
                console.error("parserStringNasted :"+ template,error);
            }
    ...
    public parserString(str:string, data:any): any {
    ...
    public parserWithFunction(data: any, key: string): string {
...
//#region Functions
    ...
    private intToBool = (params:any[]) => {
        ...
        if(params[0]=='false' || params[0] === '' || params[0] == "0" || params[0] == undefined || params[0] == null)
            return 'false';
    ...
    private intArrayConcat = (data: any, params:any[]) => {
        ...
        if(Array.isArray(array)){
            array.forEach(x=>{
                ...
                result += this.parserStringNasted(template, x, null);
            })
        }
    ...
    private intArraySum = (data: any, params:any[]) => {
        ...
        if(Array.isArray(array)){
            array.forEach(x=>{
                let template = [...params].slice(1).join('|');
                ////console.log("ArrayConcat item", x, template);
                let single = this.parserStringNasted(template, x, null);
    ...
    private intFromOtherData = (otherData:any, data:any, params:any[]) => {
        //debugger;
        if(!otherData) return null;
        if (!params || params.length > 1) return null;
        let value = this.fromContext(otherData, params[0]);
        return value;
    }

    private intJson = (params: any) => {
        //faccio console log di tutto
        console.log("intJson", params, params);
        if (!params || params.length < 2) return null;
    
        try {
            if (params[0].toLowerCase() === "parse") {
                let obj = JSON.parse(params[1]);
                if(!obj) return null;
                let path = params[2];
                if(path.startsWith('.')) path = path.substring(1); 
                let finalValue = this.fromContext(obj, path);
                return finalValue;
            }
    
            // !TODO DA FINIRE, rivedere logica funzione parserStringNasted che restituisce sempre stringa
